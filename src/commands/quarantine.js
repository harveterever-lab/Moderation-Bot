...
export const data = new SlashCommandBuilder()
  .setName('quarantine')
  .setDescription('Quarantine a member by assigning the quarantine role.')
  .addUserOption((opt) => opt.setName('user').setDescription('The member to quarantine').setRequired(true))
  .addStringOption((opt) => opt.setName('reason').setDescription('Reason for the quarantine').setRequired(true));
...
export async function execute(interaction) {
  ...
  if (target.roles.cache.has(config.quarantineRoleId)) {
    await interaction.reply({ content: '❌ This member is already quarantined.', ephemeral: true });
    return;
  }
  ...
  try {
    ...
    await interaction.reply({ content: `✅ Successfully quarantined ${targetUser}.` });
    ...
    await interaction.reply({ content: '❌ Something went wrong while trying to quarantine that member.', ephemeral: true });
  }
}
